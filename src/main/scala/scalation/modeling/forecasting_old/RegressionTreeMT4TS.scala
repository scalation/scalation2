...
class RegressionTreeMT4TS (x: MatrixD, yy: VectorD, lags: Int, fname: Array [String] = null,
    ...
    modelName = s"RegressionTreeMT4TS_$lags"
...
object RegressionTreeMT4TS:
    ...
    def exo (y: VectorD, lags: Int, ex: MatrixD, h: Int,
            ...
            (elag1: Int = max (1, lags / 5),
             elag2: Int = max (1, lags)): RegressionTreeMT4TS =
        val (x_, yy) = buildMatrix4TS (y, lags, h)                        // column for each lag
...
/*
    ...
    new PlotM (null, rSq.transpose, Array ("R^2", "R^2 bar", "sMAPE", "NA"),
               s"R^2 vs n for RegressionTreeMT4TS with tech", lines = true)
//  println (mod.summary ())
...
@main def regressionTreeMT4TSTest5 (): Unit =
    ...
    val mod = RegressionTreeMT4TS.exo (y, LAGS, ex, hh)(1, LAGS+1)          // create model for time series data - with exo
    val (yp, qof) = mod.trainNtest ()()                                     // train on full and test on full
