...
class ARX_Symb_D (x: MatrixD, y: MatrixD, hh: Int, n_exo: Int, fname: Array [String],
                  ...
                  tForms: TransformMap = Map ("tForm_y" -> null))        // backcast value used only `MakeMatrix4TS`
...
object ARX_Symb_D:
    ...
    def apply(xe: MatrixD, y: VectorD, hh: Int, fname_ : Array[String] = null,
        ...
        val fname = if fname_ == null then ARX_Symb.formNames(xe.dim2, hparam, fEndo.length, fExo.length) else fname_
