...
//      println (s"exo: x = $x \n yy = $yy")
        val mod = new RegressionMV (x, yy, null, hparam)
        mod.modelName = s"RegressionMV4TS.exo_$lag"
        mod
    end exo
     ...
     *  @param h    the forecasting horizon (1, 2, ... h)
     *
    def buildMatrix (y: VectorD, lag: Int, h: Int): (MatrixD, MatrixD) =
...
@main def regressionMV4TSTest3 (): Unit =
    ...
    val mod = RegressionMV4TS (y, 10, 4)                                    // create model for time series data
...
@main def regressionMV4TSTest4 (): Unit =
    ...
    val mod = RegressionMV4TS.exo (y, 10, ex, 4)(1, 11)                     // create model for time series data - with exo
    val (yp, qof) = mod.trainNtest ()()                                     // train on full and test on full
    val yy_ = y(10 until y.dim)
    new Plot (null, yy_, yp(?, 0), s"${mod.modelName}, yy vs. yp", lines = true)
...
@main def regressionMV4TSTest6 (): Unit =
    ...
    banner ("Test In-Sample RegressionMV4TS.exo on COVID-19 Weekly Data")
    val mod = RegressionMV4TS.exo (y, MX_LAGS, ex, 4)(1, MX_LAGS+1)         // create model for time series data with exo
    val (yp, qof) = mod.trainNtest ()()                                     // train on full and test on full
    val yy_ = y(MX_LAGS until y.dim)
    new Plot (null, yy_, yp(?, 0), s"${mod.modelName}, yy vs. yp", lines = true)
