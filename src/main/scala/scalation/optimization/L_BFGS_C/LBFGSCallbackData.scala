 ...
 *  [[EvaluationLogicNative]] specified for a L-BFGS optimization done by the
 *  [[Native]] object with values that are the parameters for the methods of the
 *  [[EvaluationLogicNative]]. This allows the user to pass the optimization
 ...
 *  @param instance         User data provided for a given call of the L-BFGS
 *                          optimization done by `lbfgsMain` on the [[Native]]
 *                          object. Can have [[Any]] type defined by the user as
 *                          long as it is the same one expected by the
 *                          `optimizationLogic` parameter.
 *  @param evaluationLogic  [[EvaluationLogicNative]] that describes the
 *                          optimization steps for the L-BFGS optimization done
 *                          by the [[Native]] object.
 */
case class LBFGSCallbackData(
    n: Int,
    instance: Any,
    evaluationLogic: EvaluationLogicNative
)
