 ...
 *
 *  @note    Example Model: Traffic using RV for Motion for Process-Interaction Simulation
 */
 ...
 *  It used `Variate` for motion, e.g., a `Uniform` Random Variate.
...
class TrafficModel (name: String = "Traffic", reps: Int = 1, animating: Boolean = true,
    ...
    val moveRV     = Uniform (mvTime, stream + 3)       // use Random Variate (RV) for Motion
