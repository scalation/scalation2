...
case class Random2 (stream: Int = 0)
    ...
    inline def gen: Double =
        ...
        if y1 < 0 || y1 >= M1 then y1 -= M1    //must also check overflow
        y1 += x13
        if y1 < 0 || y1 >= M1 then y1 -= M1
        x13 = x12; x12 = x11; x11 = y1

        // Calculate MRG2 (second component)

        y1 = ((x21 & MASK21) << 15) + (MULT2 * (x21 >>> 16))
        if y1 < 0 || y1 >= M2 then y1 -= M2
        var y2 = ((x23 & MASK21) << 15) + (MULT2 * (x23 >>> 16))
        if y2 < 0 || y2 >= M2 then y2 -= M2
        y2 += x23
        if y2 < 0 || y2 >= M2 then y2 -= M2
        y2 += y1
        if y2 < 0 || y2 >= M2 then y2 -= M2
    ...
    inline def igen: Int =
        ...
        if x11 <= x21 then x11 - x21 + M1 else x11 - x21
    end igen
