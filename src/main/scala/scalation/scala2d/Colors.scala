 ...
 *
 *  @note    Definitions for Numerous Common Colors
 */
...
object Colors:
    ...
    def randomColor (id: Int, full: Boolean = true): Color =
        ...
        val bases = if full then 27 else 25
        id % bases match
 ...
 */
@main def colorsTest (): Unit =

    import Colors.randomColor
    for i <- 0 until 20 do println (s"color $i = ${randomColor (i)}")
